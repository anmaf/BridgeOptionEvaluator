...
col1 = [[sg.Text('S hand', font=fontAb)], \
        [sg.Listbox(values = BN_form_card_list, no_scrollbar = True, select_mode=sg.LISTBOX_SELECT_MODE_MULTIPLE, \
                    font=fontA, size=card_list_size, key = 'S_hand')]]
col2 = [[sg.Text('N hand', font=fontAb)], \
        [sg.Listbox(values = BN_form_card_list, no_scrollbar = True, select_mode=sg.LISTBOX_SELECT_MODE_MULTIPLE, \
                    font=fontA, size=card_list_size, key = 'N_hand')]]
...
layout = [[sg.Column(col1, element_justification='c'), sg.VSeparator(color='grey'), \
          ...
          [sg.Output(size=output_window_size, echo_stdout_stderr = echo_in_console,  font=fontA, key='OUTPUT')]]

window = sg.Window(
    'Bridge Option Evaluator', layout, default_button_element_size=default_button_size, font=fontA)